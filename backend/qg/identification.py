...
class Identification:
    @staticmethod
    def clause_identify(segmented_sentence):
        tok = nltk.word_tokenize(segmented_sentence)
    ...
    @staticmethod
    def subject_phrase_search(segmented_sentence, index):
        str2 = ""
        for j in range(index - 1, 0, -1):
            str1 = ""
            flag = 0
            tok = nltk.word_tokenize(segmented_sentence[j])
            tag = nltk.pos_tag(tok)
            gram = r"""chunk:{<EX>?<DT>?<JJ.?>*<NN.?|PRP|PRP\$|POS|IN|DT|CC|VBG|VBN>+<RB.?>*<VB.?|MD|RP>+}"""
            chunkparser = nltk.RegexpParser(gram)
            chunked = chunkparser.parse(tag)

            list1 = Identification.chunk_search(segmented_sentence[j], chunked)
            if len(list1) != 0:
                m = list1[len(list1) - 1]
            ...
            if flag == 0:
                tok1 = nltk.word_tokenize(segmented_sentence[j])
                tag1 = nltk.pos_tag(tok1)
                gram1 = r"""chunk:{<EX>?<DT>?<JJ.?>*<NN.?|PRP|PRP\$|POS|IN|DT|CC|VBG|VBN>+}"""
                chunkparser1 = nltk.RegexpParser(gram1)
                chunked1 = chunkparser1.parse(tag1)
                ...
                list2 = Identification.chunk_search(str1, chunked1)
                st = nltk.word_tokenize(segmented_sentence[j])
                if len(chunked1[list2[0]]) == len(st):
                    str2 = segmented_sentence[j]
                    flag = 1
    ...
    @staticmethod
    def verbphrase_identity(clause):
        ...
        str2 = ""
        str3 = ""
        list1 = Identification.chunk_search(clause, chunked)
        if len(list1) != 0:
            m = list1[len(list1) - 1]
            for j in range(len(chunked[m])):
                str1 += chunked[m][j][0]
        ...
        list3 = Identification.chunk_search(str1, chunked2)
        if len(list3) != 0:
